...
app.get('/fun', (req, res) => {
    const pt = path.join(__dirname, "./fun")
    res.setHeader('Content-Type', 'text/javascript');
    const file = fs.readFileSync(`${pt}/main.js`)
    res.send(file)
})

app.get('/fun/bin/:name', (req, res) => {
    const name = req.params.name
    const pt = path.join(__dirname, "./fun/bin")
    const dr = fs.readdirSync(`${pt}`)
    const f = dr.find((v) => v === `${name}.js`)
    res.setHeader('Content-Type', 'text/javascript');
    if (!f) return res.sendFile(path.join(__dirname, './fun/util/not-found.js'))
    const file = fs.readFileSync(`${pt}/${f}`)
    res.send(file)
})
